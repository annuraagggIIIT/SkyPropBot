...
if df is not None:
    ...
    def chat_with_property_bot(query):
        custom_query_result = process_query(query)
        ...
        conversation = chatbot(f"User: {query}")
        response = conversation[0]['generated_text']
        return response


    while True:
        query = input("Ask a question about the properties or type 'exit' to quit: ")
        if query.lower() == 'exit':
            break
        response = chat_with_property_bot(query)
